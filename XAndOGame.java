...
//// initialize array with its element  ..  char [][] gBoard = new char[][]; or 
	...
	//player 1 Turn 
	//accept input from user and save it in userInput
	...
	//If userInput is Invalid it will display a message "Invalid Number"
		private static void playerTurn1(char[][] gBoard, Scanner sc) {
			int userInput;
			while (true) {
				System.out.println("Player1 .. Select number (0-8):");
				userInput = sc.nextInt();
				if (validMove(gBoard, userInput)) {
					break;
					
				} else {
					System.out.println(userInput + " Invalid Number");
				}
			}
			move(gBoard, userInput, 'X');//Any number 0-8 which selected by player1 will placed as X
		}
	...
	//If userInput is Invalid it will display a message "Invalid Number"
		private static void playerTurn2(char[][] gBoard, Scanner sc) {
			int userInput2;
			while (true) {
				System.out.println("Player B..select number(0-8): ");
	...
	//method for checking if the place which selected by user is valid or not
